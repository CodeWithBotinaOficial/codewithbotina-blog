...
export default function PollEditor({ slug, pollLanguage }: Props) {
  ...
  async function save() {
    ...
    if (!title.trim()) {
      showToast(
        t(lang, "polls.createModal.errors.titleRequired", "admin"),
        "error",
      );
      return;
    }
    ...
    try {
      ...
      showToast(
        err instanceof Error
          ? err.message
          : t(lang, "polls.editModal.saveFailed", "admin"),
        "error",
      );
    } finally {
      setSaving(false);
  ...
  async function addOption() {
    ...
    try {
      await pollsApi.addOption(currentSlug, pollLanguage, {
        option_text: text,
      });
      setNewOptionText("");
      ...
      showToast(
        err instanceof Error
          ? err.message
          : t(lang, "polls.editModal.addOptionFailed", "admin"),
        "error",
      );
    } finally {
  ...
  async function deleteOption(optionId: string) {
    if (!confirm(t(lang, "polls.editModal.deleteOptionConfirm", "admin")))
      return;
    try {
      ...
      showToast(
        t(lang, "polls.editModal.deleteOptionFailed", "admin"),
        "error",
      );
    }
  }
  ...
  if (loading)
    return (
      <div class="text-sm text-[var(--color-text-secondary)]">
        {t(lang, "polls.editModal.loading", "admin")}
      </div>
    );
  if (!poll)
    return (
      <div class="rounded-xl border border-[var(--color-border)] bg-white p-6 text-sm">
        {t(lang, "polls.editModal.loadFailed", "admin")}
      </div>
    );
  ...
  return (
    <div class="space-y-6">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 class="text-2xl font-bold">
            {t(lang, "polls.editModal.title", "admin")}
          </h1>
          <p class="text-sm text-[var(--color-text-secondary)]">
            <span class="font-mono text-xs">{poll.slug}</span> · {poll.language}{" "}
            · {poll.type}
          </p>
        </div>
        <button
          ...
          onClick={save}
          disabled={saving || !slugIsValid}
        >
          <Save className="h-4 w-4" />
          {saving
            ? t(lang, "polls.editModal.saving", "admin")
            : t(lang, "polls.editModal.save", "admin")}
        </button>
      </div>
      ...
      <div class="grid grid-cols-1 gap-4 rounded-xl border border-[var(--color-border)] bg-white p-5 md:grid-cols-2">
        ...
        <div class="space-y-2">
          <label class="text-sm font-semibold">
            {t(lang, "polls.table.status", "admin")}
          </label>
          <select
            class="input-field"
            value={status}
            onChange={(e) =>
              setStatus((e.currentTarget as HTMLSelectElement).value as any)
            }
          >
            <option value="open">
              {t(lang, "polls.status.open", "admin")}
            </option>
            <option value="closed">
              {t(lang, "polls.status.closed", "admin")}
            </option>
          </select>
        </div>
        ...
        <div class="space-y-2">
          ...
          <input
            ...
            value={closesAt}
            onInput={(e) =>
              setClosesAt((e.currentTarget as HTMLInputElement).value)
            }
          />
          <div class="text-xs text-[var(--color-text-tertiary)]">
            {t(lang, "polls.editModal.closesAtHelp", "admin")}
          </div>
        </div>
      </div>
      ...
      {poll.type !== "free_text" ? (
        <div class="rounded-xl border border-[var(--color-border)] bg-white p-5">
          <h2 class="text-lg font-bold mb-3">
            {t(lang, "polls.editModal.options", "admin")}
          </h2>
          ...
          <div class="space-y-2">
            {opts.map((o: any) => (
              <div
                key={o.id}
                class="flex items-center justify-between gap-3 rounded-xl border border-[var(--color-border)] px-4 py-3"
              >
                <div class="min-w-0">
                  ...
                  <div class="text-xs text-[var(--color-text-tertiary)]">
                    {t(lang, "polls.editModal.order", "admin")}:{" "}
                    {o.display_order}
                  </div>
                </div>
                <button
                  ...
                  title={t(lang, "polls.editModal.deleteOption", "admin")}
                >
          ...
          <div class="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center">
            <input
              ...
              placeholder={t(
                lang,
                "polls.editModal.newOptionPlaceholder",
                "admin",
              )}
              value={newOptionText}
              onInput={(e) =>
                setNewOptionText((e.currentTarget as HTMLInputElement).value)
              }
            />
            <button
              type="button"
            ...
            >
              ...
              {addingOption
                ? t(lang, "polls.editModal.addingOption", "admin")
                : t(lang, "polls.editModal.addOption", "admin")}
            </button>
          </div>
        </div>
      ) : null}
      ...
      <div class="rounded-xl border border-[var(--color-border)] bg-white p-5">
        <h2 class="text-lg font-bold mb-2">
          {t(lang, "polls.editModal.embed", "admin")}
        </h2>
        <div class="text-sm text-[var(--color-text-secondary)]">
          {t(lang, "polls.editModal.embedHelp", "admin")}
          <pre class="mt-2 rounded-lg bg-[var(--color-bg-subtle)] p-3 text-xs overflow-x-auto">
            <code>
              [{poll.title}](poll:{editedSlug || poll.slug})
            </code>
          </pre>
        </div>
      </div>
      ...
      <PollTranslationLinker
        pollSlug={currentSlug}
        pollType={poll.type}
        pollLanguage={poll.language ?? pollLanguage}
