...
\restrict w9DqbGqigoIW0chl7JBy2lROxybWpkqggQPassDdamO0HsUJPVAii34Sq5duorg
...
DROP INDEX IF EXISTS storage.objects_bucket_id_name_version_key;
DROP INDEX IF EXISTS storage.name_prefix_search;
DROP INDEX IF EXISTS storage.idx_objects_null_version;
DROP INDEX IF EXISTS storage.idx_objects_current_version;
...
DROP INDEX IF EXISTS public.idx_posts_status_scheduled_at;
DROP INDEX IF EXISTS public.idx_posts_slug;
DROP INDEX IF EXISTS public.idx_posts_scheduled_at;
...
CREATE INDEX idx_posts_scheduled_at ON public.posts USING btree (scheduled_at) WHERE (((status)::text = 'scheduled'::text) AND (scheduled_at IS NOT NULL));
...
CREATE INDEX idx_posts_status_scheduled_at ON public.posts USING btree (status, scheduled_at DESC) WHERE (scheduled_at IS NOT NULL);
...
CREATE POLICY "Admins can read all posts" ON public.posts FOR SELECT TO authenticated USING ((EXISTS ( SELECT 1
...
--
-- Name: posts Public can read published posts; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY "Public can read published posts" ON public.posts FOR SELECT USING (((status)::text = 'published'::text));
...
\unrestrict w9DqbGqigoIW0chl7JBy2lROxybWpkqggQPassDdamO0HsUJPVAii34Sq5duorg
