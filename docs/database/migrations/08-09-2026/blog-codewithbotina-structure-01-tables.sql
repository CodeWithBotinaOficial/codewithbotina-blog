...
\restrict dmWXiZqs5H7vMTkafVLrPcvK5WkCaCb3M3qJFM4h02cdL5owGiZfLYVO3MAAZMY
...
DROP TABLE IF EXISTS public.poll_posts;
DROP TABLE IF EXISTS public.poll_display_settings;
DROP VIEW IF EXISTS public.mismatched_poll_translations;
DROP TABLE IF EXISTS public.polls;
DROP TABLE IF EXISTS public.poll_options;
DROP TABLE IF EXISTS public.cookie_consent;
...
DROP FUNCTION IF EXISTS public.validate_top_count();
...
DROP FUNCTION IF EXISTS public.prevent_delete_option_with_votes();
DROP FUNCTION IF EXISTS public.handle_new_user();
DROP FUNCTION IF EXISTS public.get_translation_for_language(input_post_id uuid, target_language text);
DROP FUNCTION IF EXISTS public.get_post_translations(input_post_id uuid);
DROP FUNCTION IF EXISTS public.enforce_poll_translation_type_match();
DROP FUNCTION IF EXISTS public.delete_user_account(target_user_id uuid);
DROP FUNCTION IF EXISTS public.create_translation_group(input_post_ids uuid[]);
...
--
-- Name: create_translation_group(uuid[]); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.create_translation_group(input_post_ids uuid[]) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
...
--
-- Name: enforce_poll_translation_type_match(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.enforce_poll_translation_type_match() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
  expected_type text;
...
--
-- Name: get_post_translations(uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.get_post_translations(input_post_id uuid) RETURNS TABLE(post_id uuid, language text)
    LANGUAGE sql STABLE
    AS $$
  WITH grp AS (
...
--
-- Name: get_translation_for_language(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.get_translation_for_language(input_post_id uuid, target_language text) RETURNS uuid
    LANGUAGE sql STABLE
    AS $$
  WITH grp AS (
    SELECT translation_group_id
...
--
-- Name: prevent_delete_option_with_votes(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.prevent_delete_option_with_votes() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
vote_count INTEGER;
...
--
-- Name: set_comment_translation_group(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.set_comment_translation_group() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
--
-- Name: sync_comments_translation_group_from_post_link(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.sync_comments_translation_group_from_post_link() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
...
--
-- Name: validate_top_count(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.validate_top_count() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
...
COMMENT ON TABLE public.admin_users IS 'Whitelist of admin users';
...
CREATE TABLE public.comments (
    ...
    CONSTRAINT comments_content_check CHECK (((length(content) >= 10) AND (length(content) <= 1000)))
);
...
COMMENT ON COLUMN public.comments.content IS 'Comment text (10-1000 characters)';


--
-- Name: COLUMN comments.is_pinned; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.comments.is_pinned IS 'True if comment is pinned by admin (appears first)';
...
CREATE TABLE public.posts (
    ...
    CONSTRAINT posts_status_check CHECK (((status)::text = ANY ((ARRAY['draft'::character varying, 'published'::character varying, 'scheduled'::character varying])::text[]))),
    CONSTRAINT valid_language CHECK (((language)::text = ANY ((ARRAY['en'::character varying, 'es'::character varying, 'fr'::character varying, 'de'::character varying, 'pt'::character varying, 'pt-br'::character varying, 'ja'::character varying, 'zh'::character varying])::text[])))
);


--
-- Name: COLUMN posts.language; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.posts.language IS 'Post language: en, es, fr, de, pt, pt-br, ja, zh';
...
COMMENT ON COLUMN public.posts.is_pinned IS 'Indicates if post is pinned/featured. Pinned posts appear first on homepage.';


--
-- Name: comments_with_language; Type: VIEW; Schema: public; Owner: -
--

CREATE VIEW public.comments_with_language WITH (security_invoker='on') AS
 SELECT c.id,
    c.post_id,
    c.user_id,
    c.content,
...
--
-- Name: cookie_consent; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.polls (
    ...
    updated_at timestamp with time zone DEFAULT now(),
    translation_group_id uuid,
    CONSTRAINT polls_language_check CHECK (((language)::text = ANY ((ARRAY['es'::character varying, 'en'::character varying, 'pt-br'::character varying])::text[]))),
    CONSTRAINT polls_status_check CHECK (((status)::text = ANY ((ARRAY['open'::character varying, 'closed'::character varying])::text[]))),
    CONSTRAINT polls_type_check CHECK (((type)::text = ANY ((ARRAY['free_text'::character varying, 'single_choice'::character varying, 'multiple_choice'::character varying])::text[])))
);


--
-- Name: COLUMN polls.translation_group_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.polls.translation_group_id IS 'Links translated polls to a root poll id. NULL if standalone.';


--
-- Name: mismatched_poll_translations; Type: VIEW; Schema: public; Owner: -
--

CREATE VIEW public.mismatched_poll_translations WITH (security_invoker='on') AS
 WITH option_counts AS (
         SELECT poll_options.poll_id,
            (count(*))::integer AS option_count
           FROM public.poll_options
          GROUP BY poll_options.poll_id
        )
 SELECT p1.id AS poll_1_id,
   ...
   FROM (((public.polls p1
     JOIN public.polls p2 ON ((p1.translation_group_id = p2.translation_group_id)))
     LEFT JOIN option_counts oc1 ON ((oc1.poll_id = p1.id)))
     LEFT JOIN option_counts oc2 ON ((oc2.poll_id = p2.id)))
  WHERE ((p1.id < p2.id) AND (p1.translation_group_id IS NOT NULL) AND ((p1.type)::text = ANY ((ARRAY['single_choice'::character varying, 'multiple_choice'::character varying])::text[])) AND ((p2.type)::text = ANY ((ARRAY['single_choice'::character varying, 'multiple_choice'::character varying])::text[])) AND (COALESCE(oc1.option_count, 0) <> COALESCE(oc2.option_count, 0)));


--
-- Name: poll_display_settings; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.poll_display_settings (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    poll_id uuid NOT NULL,
    show_top boolean DEFAULT false,
    top_count integer,
    top_order character varying(10),
    show_bar_chart boolean DEFAULT false,
    bar_chart_orientation character varying(20),
    bar_chart_options_count integer,
    CONSTRAINT poll_display_settings_bar_chart_orientation_check CHECK (((bar_chart_orientation)::text = ANY ((ARRAY['horizontal'::character varying, 'vertical'::character varying])::text[]))),
    CONSTRAINT poll_display_settings_top_order_check CHECK (((top_order)::text = ANY ((ARRAY['asc'::character varying, 'desc'::character varying])::text[])))
);


--
-- Name: poll_posts; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.poll_posts (
    poll_id uuid NOT NULL,
    post_id uuid NOT NULL,
    created_at timestamp with time zone DEFAULT now()
);
...
CREATE TABLE public.poll_votes (
    ...
    updated_at timestamp with time zone DEFAULT now(),
    translation_group_id uuid,
    CONSTRAINT poll_votes_check CHECK ((((free_text_response IS NOT NULL) AND (poll_option_id IS NULL)) OR ((free_text_response IS NULL) AND (poll_option_id IS NOT NULL))))
);


--
-- Name: COLUMN poll_votes.translation_group_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.poll_votes.translation_group_id IS 'Effective poll translation group for unified vote counting. Standalone votes use poll_id.';
...
CREATE TABLE public.post_reactions (
    ...
    CONSTRAINT post_reactions_reaction_type_check CHECK ((reaction_type = ANY (ARRAY['like'::text, 'dislike'::text])))
);


--
-- Name: TABLE post_reactions; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.post_reactions IS 'Likes and dislikes on blog posts';


--
-- Name: COLUMN post_reactions.translation_group_id; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.post_reactions.translation_group_id IS 'Effective post translation group. For unlinked posts this is the post_id.';
...
CREATE TABLE public.post_translations (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT post_translations_language_check CHECK ((language = ANY (ARRAY['en'::text, 'es'::text, 'fr'::text, 'de'::text, 'pt'::text, 'pt-br'::text, 'ja'::text, 'zh'::text])))
);


--
-- Name: TABLE post_translations; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN post_translations.translation_group_id; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: COLUMN post_translations.language; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON COLUMN public.post_translations.language IS 'Language code of the linked post (en, es, fr, de, pt, pt-br, ja, zh)';


--
-- Name: posts_backup; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: TABLE posts_backup; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.posts_backup IS 'Backup created before i18n migration on 2026-03-06 01:08:17';
...
COMMENT ON TABLE public.users IS 'User profiles populated from Google OAuth';
...
\unrestrict dmWXiZqs5H7vMTkafVLrPcvK5WkCaCb3M3qJFM4h02cdL5owGiZfLYVO3MAAZMY
