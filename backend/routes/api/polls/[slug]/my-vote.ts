...
export const handler: Handlers = {
  ...
  async GET(req, ctx) {
    ...
    try {
      ...
      if (!user) {
        const response = successResponse(null, "No user", 200);
      ...
      const votes = await pollRepository.getUserVote(poll.id, user.id);
      const response = successResponse(votes, "User votes", 200);
      headers.forEach((value, key) => response.headers.set(key, value));
      return response;
    } catch (error) {
